...
### 자막 생성 (SRT 형식)
- **목적**: YouTube Shorts는 음소거 시청 비율 높음 → 자막 필수 (특히 고전시가)
- **타이밍**: Step 3에서 오디오 생성 시 자막도 함께 생성 (오디오 길이가 확정되므로)
- **포맷**: SRT (SubRip, MoviePy/YouTube 모두 지원)
- **자막 텍스트**: `narration` 필드 사용 (TTS 음성과 정확히 일치)
- **캐시**: 오디오 경로 목록을 MD5 해시 → `cache/step3/{hash8}_subtitles.srt`
- **함수**:
...
### Step 3 → Step 4 인터페이스
- `generate_all_audio()` 반환값: `dict{'audio_paths': list[str], 'subtitle_path': str}` (변경됨)
  - `audio_paths`: 오디오 경로 목록 (실패 씬은 빈 문자열)
  - `subtitle_path`: SRT 자막 파일 경로 (Step 4 MoviePy에 전달)
...
### 오류 3: generate_all_audio() 반환값 변경
Step 3 완료 후 오디오 경로만 반환 → Step 4에서 자막 경로를 찾아야 함 (비효율)
**→ `generate_all_audio()`가 dict 반환: `{'audio_paths': [...], 'subtitle_path': '...'}`**
**변경 영향**: `step3_audio.py`의 `main()` 함수만 수정 필요 (외부 호출처 없음)
**확인**: 반환값 dict 구조 검증 + 2회 연속 실행 시 "자막 캐시 사용:" 로그 확인
