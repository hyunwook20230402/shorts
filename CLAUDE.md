...
**현재 상태:** Step 0~6 전체 구현 완료 (ElevenLabs TTS + ComfyUI Flux.1-dev FP8 정지이미지 + NanumSquare 자막 + Stable Audio BGM + 15개 이중 테마 시스템 + 6개 지배적 정서).
...
- **음성:** ElevenLabs TTS (`eleven_multilingual_v2`, 여성/남성 voice ID 2종)
...
Step 2: 음성+타임스탬프  ElevenLabs TTS로 문장별 MP3 + alignment(추정 타임스탬프) JSON
...
Step 6: 최종 병합        이미지+오디오+자막+BGM 슬라이드쇼 (1080×1920, 30fps)
                        --no-bgm 옵션 시 shorts_no_bgm.mp4 (TTS+자막만) 별도 출력
```

**이중 테마 시스템:** 15개 고전시가 테마 (A~M, B1/B2·F1/F2 이분화).
- `primary_theme`: 작품의 근본 주제 (시상/메시지)
- `surface_theme`: 시각적으로 드러나는 배경/소재 (이미지 프롬프트에 사용)
- `dominant_emotion`: 지배적 정서 코드 (E1~E6, BGM/이미지 분위기에 반영)
...
- `dominant_emotion` / `dominant_emotion_en` — 지배적 정서 (E1~E6)
- `modern_script_data` — 씬별 데이터 배열 (image_prompt, pose_type, composition, main_focus 포함)

**환경변수 (`.env`):**
```
# API 키
NCP_CLOVA_API_KEY         # HCX-005 OCR/번역/이미지 프롬프트
...
**Step 1 (NLP):** HCX-005 번역 + 이중 테마/정서 분류 + HCX-005 이미지 프롬프트 + composition(8종)/pose_type(18종) 생성, 캐시 `{poem_dir}/step1/nlp.json`

**Step 2 (ElevenLabs TTS):** `eleven_multilingual_v2` 모델, 문장 단위 MP3 + alignment JSON. Free 플랜은 with-timestamps 미지원이라 mutagen 기반 MP3 길이 측정 + 단어/문장 균등 분배로 alignment 추정. 캐시 `{poem_dir}/step2/scene{NN}_sent{MM}_audio.mp3|_alignment.json`

**Step 3 (문장 스케줄링):** 문장 단위 duration/prompt/audio_path 매핑, 캐시 `{poem_dir}/step3/sentence_schedule.json`

**Step 4 (Flux.1-dev FP8 정지이미지):** GuoFeng5 LoRA + seed 랜덤화 + composition guard + 시대 고증 키워드, 캐시 `{poem_dir}/step4/scene{NN}_sent{MM}_still.png`

**Step 5 (BGM):** Stable Audio 기반 BGM 생성, 테마별 악기/분위기 + 지배적 정서 LLM 프롬프트 주입, 캐시 `{poem_dir}/step5/bgm.wav`

**Step 6 (병합):** 이미지+오디오 슬라이드쇼 + NanumSquare 자막 burn-in (흰색+검은 외곽선, 65% 위치, 어절 단위 줄바꿈) + BGM 믹싱, 캐시 `{poem_dir}/step6/shorts.mp4`. CLI `--no-bgm` 옵션 시 BGM 믹싱 스킵 후 `shorts_no_bgm.mp4`로 별도 출력.
...
| PRD 작성 필요 | `prd-writer-shorts` | 파이프라인 PRD 문서 작성 |
| PRD 검증 필요 | `prd-validator` | PRD 완전성·실현가능성 검증 |
...
- `step2_tts.py` — Step 2 ElevenLabs TTS MP3 + alignment JSON (mutagen 기반 길이 측정)
...
- `theme_config.py` — 15개 테마 + 6개 정서 설정 단일 소스
...
| `cache-field-checklist.md` | JSON 필드 추가 시 E2E 검증, use_cache 규칙, 이중 쓰기 방지 |

---

**마지막 업데이트:** 2026-04-25 (Step 2 ElevenLabs TTS 전환 + Step 6 `--no-bgm` 옵션 + Step 0 OCR 반복행 보존 강화)
