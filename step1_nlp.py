...
- modern_text: 현대어 번역 (자연스러운 현대 한국어, 단어 풀이 아닌 문장 번역)
...
def validate_scene(raw_scene: dict, idx: int, historical_context: str = '') -> dict:
  ...
  return {
    ...
    'narration': raw_scene.get('narration', raw_scene.get('modern_text', '')),
    'emotion': raw_scene.get('emotion', '미정'),
    'background': raw_scene.get('background', ''),
    'image_prompt': '',  # call_hcx005_image_prompt 호출 후 채움
    'historical_context': historical_context,  # 역사적 배경 저장
  }
