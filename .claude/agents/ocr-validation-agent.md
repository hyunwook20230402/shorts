...
### 5. 행 경계 정확성 검증 (Line Boundary Validation)

**줄 수가 일치하더라도 단어가 잘못된 줄에 배정될 수 있습니다.** 이 검증은 줄 수 검증을 통과한 경우에도 반드시 수행해야 합니다.

**검증 포인트:**
- `notebook/ocr_postcheck.py`의 `detect_suspicious_lines()` 결과를 **반드시** 참조하고, 의심 행이 있으면 이미지와 대조
- 행 시작이 한자 병기 단어(`한글(漢字)`)로 시작하고 뒤에 다른 구절이 이어지는 경우 → 이전 행에서 밀려왔을 가능성 경고
- 이미지를 직접 읽어 해당 단어가 실제로 어느 행에 속하는지 시각적 확인
- **줄 수가 맞아도 행 경계가 잘못되면 FAIL 판정**

**실제 사례 (2026-04-07):**
- `경경(耿耿)`이 `고침상(孤枕上)*에` 행에서 다음 행 `서창(西窓)을` 앞으로 밀림
- 줄 수 11줄로 동일하여 줄 수 검증 통과했으나, 행 경계 오류로 NLP 씬 분할이 잘못됨
...
`notebook/ocr_postcheck.py`의 `detect_suspicious_lines()`를 호출하여 행 경계 오류 의심 행을 감지합니다. 의심 행이 있으면 이미지와 대조하여 실제 행 소속을 확인합니다.

```python
from ocr_postcheck import detect_suspicious_lines
suspicious = detect_suspicious_lines(ocr_text)
# suspicious가 비어있지 않으면 이미지와 대조 필수
```
