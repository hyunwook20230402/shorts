...
**파이프라인 구조:** Step 0~5 (6단계, v2)
- Step 0: OCR (HCX-005)
- Step 1: NLP (gpt-4o-mini)
- Step 2: 음성+타임스탬프 (ElevenLabs TTS)
- Step 3: 프레임 스케줄 (BatchPromptSchedule)
- Step 4: 영상 클립 (ComfyUI AnimateDiff SD1.5+국풍LoRA)
- Step 5: 최종 병합 (MoviePy burn-in + 오디오 싱크)
...
4. **단계별 요구사항 및 구현 지침** — 각 Step 0~5의 입력/처리/출력/제약조건 (v2 기준)
...
- [ ] AnimateDiff BatchPromptSchedule 프롬프트 삽입 조건이 명시되었는가?
- [ ] 영상 해상도(AnimateDiff 클립 생성 → 1080×1920 최종 업스케일) 명시되었는가?
- [ ] **poem_translation_log DB 로깅이 Step 1에 포함되는가?**
- [ ] **데이터 플라이휠 전략(3가지 파인튜닝 구간)이 명시되었는가?**
- [ ] **Notion Database 연동이 명시되었는가?**
- [ ] **Step 4 (AnimateDiff 클립 생성)이 독립 모듈로 포함되는가?**
- [ ] **Step 2~4 캐시 구조가 명시되었는가?** (cache/step2/ MP3+JSON, cache/step3/ schedule.json, cache/step4/ MP4)
