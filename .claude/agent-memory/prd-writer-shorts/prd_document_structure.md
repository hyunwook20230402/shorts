...
3. **데이터베이스 스키마** — Supabase PostgreSQL 테이블 정의
4. **단계별 요구사항 및 구현 지침** — 각 Step의 입력/처리/출력/제약조건
...
- [ ] **poem_translation_log DB 로깅이 Step 1에 포함되는가?** (新)
- [ ] **데이터 플라이휠 전략(3가지 파인튜닝 구간)이 명시되었는가?** (新)
- [ ] **Supabase 연동 요구사항이 명시되었는가?** (新)
