...
### Step 0 (OCR)
- `step0/ocr.txt` 전문 출력 (원문 텍스트)

### Step 1 (NLP)
- `step1/nlp.json` 읽어서 요약 테이블:
```
| 씬 | 원문 (앞 20자) | 감정 | 배경 |
|----|---------------|------|------|
| 1  | 청산리 벽계수야... | 비장 | 산수화 |
...
```
- 총 씬 수, 제목, 작가 정보
- primary_theme / surface_theme / dominant_emotion 값 출력
...
### Step 3 (스케줄링)
- `step3/sentence_schedule.json` 읽어서 요약:
...
### Step 5 (BGM)
- `step5/bgm.wav` 파일 크기 + 재생 길이
- LLM 생성 프롬프트 (로그에서 추출)
- 테마/정서 기반 프롬프트 일치 여부

### Step 6 (최종 병합)
- `step6/shorts.mp4` 파일 크기
- 영상 사양 안내 (1080x1920, 30fps)
- BGM 포함 여부, 자막 스타일(NanumSquare + 흰색+외곽선)
- 완성 상태 확인
