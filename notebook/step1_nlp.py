...
{"primary_theme":"A~M 코드","primary_theme_en":"영문키","surface_theme":"A~M 코드","surface_theme_en":"영문키","title":"...","author":"...","total_scenes":N,"scenes":[{"scene_index":1,"original_text":"...","modern_text":"...","narration":"...","emotion":"...","main_focus":"...","visual_elements":{"background":"...","character":"...","object":"...","atmosphere":"..."},"image_prompt":""}]}
"""
...
def process_nlp(
  ...
  if use_cache:
    ...
    if cached is not None:
      # 오래된 캐시 호환: primary_theme 없으면 theme 필드 기반으로 보충 후 재저장
      if 'primary_theme' not in cached:
        ...
        logger.info('캐시에 primary/surface_theme 필드 보충 완료: %s', old_theme)
  ...
  try:
    ...
    logger.info(
      '씬 분할 완료: %d씬, primary=%s(%s), surface=%s(%s)',
      len(raw_scenes),
...
# 7. 각 씬(문장)의 이미지 프롬프트 생성
    ...
    for i, scene in enumerate(validated_scenes):
        ...
        try:
            # LLM 호출하여 영문 프롬프트 생성
            prompt = call_hcx005_image_prompt(sentence_scene_ctx, historical_context, surface_theme)
