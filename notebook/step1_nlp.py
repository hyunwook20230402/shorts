...
## 프롬프트 생성 규칙
1. 원문(original_text)의 핵심 행동·장면을 최우선으로 묘사하세요. 원문에 "엎드려", "흙투성이", "아전" 같은 구체적 행동/인물이 있으면 반드시 이미지에 포함하세요.
...
3. 감정(emotion)을 lighting과 color tone으로 표현하세요:
   ...
   - 자긍심: warm earthy tones, upright confident posture, steady calm lighting
   - 희망/기쁨: bright green/gold tones, warm open sunlight, lively atmosphere
   - 호소/슬픔: misty soft atmosphere, gray overcast sky, pleading gestures
   - 고단함: hunched posture, muted gray-blue tones, heavy damp atmosphere
   - 비장: dramatic chiaroscuro, deep saturated tones, powerful expression
4. **시대 일관성 (필수)**: 모든 물건, 도구, 건축물, 의복은 반드시 한국 전근대(삼국~조선, ~1897) 양식만 허용.
   ...
   - 비/눈 장면: 인물이 비/눈을 직접 맞는 모습으로만 표현. 우산(umbrella/parasol) 절대 금지.
   - 서양 문물 혼합 금지: 동서양 시대 혼합 절대 불가.
5. image_prompt는 200 토큰 이내로 작성하세요.
6. 제공된 테마 스타일 가이드(theme_style_guide)의 색감/구도 지시를 반영하세요.
7. 우선순위: 원문의 구체적 행동·인물 > 시대 일관성 > 씬의 배경·계절 > 테마 스타일 가이드 > 정서 톤.
...
- landscape_only: 인물 없는 자연 풍경/배경 묘사
...
원문의 구체적 행동과 인물을 이미지에 반드시 포함하여 위 출력 형식(2줄)으로 생성하세요."""
