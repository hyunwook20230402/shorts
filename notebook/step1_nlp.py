...
8. 입력된 원문에 없는 시구나 작품명을 절대 생성하지 마세요. 오직 입력 텍스트만 번역하세요.
9. total_scenes는 입력 텍스트의 실제 구절 수와 일치해야 합니다.

## 출력 형식 (JSON만 출력, 설명 금지)
{{"primary_theme":"A~M 코드","primary_theme_en":"영문키","surface_theme":"A~M 코드","surface_theme_en":"영문키","dominant_emotion":"E1~E6 코드","dominant_emotion_en":"영문키","title":"...","author":"...","total_scenes":N,"scenes":[{{"scene_index":1,"original_text":"...","modern_text":"...","narration":"...","emotion":"...","main_focus":"...","visual_elements":{{"background":"...","character":"...","object":"...","atmosphere":"..."}},"image_prompt":""}}]}}
"""
...
def call_hcx005_translate(text: str, historical_context: str = '') -> dict:
  ...
  # 테마 분류 지시문을 동적으로 삽입
  system_content = translate_system_prompt.format(
    theme_classification_prompt=get_theme_classification_prompt(),
  )
  ...
  try:
    ...
    # 응답 잘림 감지 (finish_reason이 length면 잘린 것)
    finish_reason = result.get('choices', [{}])[0].get('finish_reason', '')
    if finish_reason == 'length':
      logger.warning('HCX-005 응답이 max_tokens 한도로 잘렸습니다. 복구 시도...')
  ...
  try:
    ...
    for chunk_idx, chunk in enumerate(chunks):
      ...
      if chunk_idx == 0:
        ...
        # 이중 테마 추출 (첫 청크에서만)
        raw_primary = result.get('primary_theme', result.get('theme', DEFAULT_THEME))
        raw_surface = result.get('surface_theme', raw_primary)
        ...
        # 지배적 정서 추출 (첫 청크에서만)
        raw_emo = result.get('dominant_emotion', DEFAULT_EMOTION)
    ...
    logger.info(
      '씬 분할 완료: %d씬, primary=%s(%s), surface=%s(%s), emotion=%s',
      len(raw_scenes),
...
# 7. 각 씬(문장)의 이미지 프롬프트 생성
    ...
    for i, scene in enumerate(validated_scenes):
        ...
        try:
            # LLM 호출하여 영문 프롬프트 생성
            prompt = call_hcx005_image_prompt(sentence_scene_ctx, historical_context, surface_theme, dominant_emotion)
