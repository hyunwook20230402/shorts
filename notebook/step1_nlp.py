...
## (중요) pose_type 분류 규칙 — main_focus에 "character"가 포함된 씬에만 적용
...
- expressive: 화자의 주장, 선언, 감정 토로, 상황 설명 등 구체적 신체 동작 없이 감정이 중심인 장면
  → 반드시 인물(화자 또는 등장인물)이 화면에 존재해야 합니다. 인물의 표정·몸짓·제스처로 감정을 표현하세요.
  → 잣, 물결, 달, 연못 등 사물/자연물이 주어인 장면에는 절대 사용하지 마세요.
  예) "그대들의 부귀 호사 우리 농부로부터 나오나니" → 화자(농부)가 말하고 있는 장면
      "아전들은 벌써부터 조세 거둔다고 성화세" → 아전들이 독촉하고 있는 장면
      "임금을 그리워하매 눈물이 절로 난다" → 화자가 울고 있는 장면
  반례) "질 좋은 잣이" → 잣(사물)이 주어 → pose_type 없음
        "달이 그림자 내린 연못 갓" → 달/연못(자연물)이 주어 → pose_type 없음

## composition 분류 규칙 — 모든 씬에 필수 (인물 유무 무관)
카메라 구도/앵글을 선택하세요. image_prompt에 해당 구도를 자연스럽게 반영하세요.

- back_view: 뒷모습, 먼 곳을 바라봄 → "viewed from behind, gazing into the distance"
- front_closeup: 정면 클로즈업, 표정/디테일 강조 → "close-up front view, face fills the frame"
- side_profile: 옆모습 실루엣 → "side profile silhouette against the sky"
- over_shoulder: 어깨 너머 시선 (두 인물 이상) → "over-the-shoulder shot, viewing from behind one figure"
- bird_eye: 부감, 위에서 내려다봄 → "bird's eye view looking down"
- low_angle: 올려다보는 앵글, 위엄/경외 → "low angle looking up"
- wide_establishing: 원경 전체샷, 풍경/공간감 → "wide establishing shot, vast landscape"
- dutch_tilt: 기울어진 구도, 불안/긴장 → "dutch angle, tilted frame"

선택 가이드:
- 감정 토로/원망/눈물 → front_closeup (표정 강조)
- 고독/사색/기다림 → side_profile 또는 back_view
- 먼 곳 조망/그리움 → back_view
- 대화/대립/바라봄 (2인 이상) → over_shoulder
- 위엄/경외/비장 → low_angle
- 풍경/자연물/공간감 → wide_establishing 또는 bird_eye
- 불안/혼란/긴장 → dutch_tilt
- ⚠️ 연속 씬에서 같은 composition 반복을 피하세요. 다양한 구도를 사용하세요.

## 출력 형식 (엄격히 준수)
- 줄1: 영어 프롬프트 텍스트만 출력. 라벨(예: "Image Prompt:", "[image_prompt]") 절대 금지.
- 줄2 (main_focus에 "character" 포함 시에만): pose_type: [위 목록 중 1개]
- 줄3 (모든 씬 필수): composition: [위 8종 중 1개]
- main_focus에 "character"가 없으면 pose_type 줄 자체를 출력하지 마세요. 빈 줄도 금지.

⚠️ 절대 금지:
- 줄1(프롬프트 텍스트) 안에 "composition", "camera angle", "shot type", "pose_type" 등 메타 용어 사용 금지.
  구도는 묘사로 자연스럽게 표현하세요 (예: "viewed from behind" ✅, "the composition focuses on..." ❌).
- "The composition focuses on...", "This shot captures...", "The camera angle..." 같은 메타 설명 문장 금지.
- pose_type/composition 줄에는 라벨과 값만 출력. 설명이나 괄호 추가 금지.
  ✅ composition: back_view
  ❌ composition: back_view (뒷모습 구도)

- 출력 예시 (인물 있는 씬):
  A lone scholar in traditional hanbok stands at the edge of a cliff, viewed from behind, gazing into the misty valley below.
  pose_type: standing_single
  composition: back_view
- 출력 예시 (인물 없는 씬):
  A solitary pine cone rests on autumn ground, wide establishing shot of fallen leaves surrounding it under cool gray overcast sky.
...
VALID_COMPOSITIONS = {
  'back_view', 'front_closeup', 'side_profile', 'over_shoulder',
  'bird_eye', 'low_angle', 'wide_establishing', 'dutch_tilt',
}
...
def call_hcx005_image_prompt(
  ...
  """HCX-005로 씬별 이미지 프롬프트 + pose_type + composition 동시 생성.
  반환: (prompt_text, pose_type, composition)
  """
  ...
  try:
    ...
    for line in lines:
      ...
      # composition 줄 파싱 (줄 시작에서만 매칭 — 자연어 "the composition..." 방지)
      mc = re.match(r'composition:?\s*(\w+)', line, re.IGNORECASE)
      if mc:
        comp_candidate = mc.group(1).strip()
        if comp_candidate in VALID_COMPOSITIONS:
          composition = comp_candidate
        else:
          logger.warning('알 수 없는 composition: %s, 기본값 사용', comp_candidate)
        cleaned = re.sub(r',?\s*composition:?\s*\w+\.?', '', line, flags=re.IGNORECASE).strip().rstrip(',').strip()
        if cleaned:
          prompt_lines.append(cleaned)
        continue
      # pose_type 줄 파싱 (줄 시작에서만 매칭)
      m = re.match(r'pose[_ ]?type:?\s*(\w+)', line, re.IGNORECASE)
      if m:
    ...
    main_focus = scene.get('main_focus', [])
    if isinstance(main_focus, str):
      main_focus = [main_focus]
    if 'character' not in main_focus and pose_type:
      logger.warning('씬 %d: main_focus에 character 없음에도 pose_type=%s 출력됨 → 강제 빈 문자열 처리', scene['scene_index'], pose_type)
      pose_type = ''

    logger.info('씬 %d 프롬프트 생성 완료 (%d자, pose=%s, comp=%s)', scene['scene_index'], len(prompt_text), pose_type, composition)
    return prompt_text, pose_type, composition
...
def process_nlp(
  ...
  try:
    ...
    # LLM이 "B2 (연군/원망)" 같이 코드 뒤에 설명을 붙이는 경우 방어: 코드 부분만 추출
    def _extract_theme_code(raw: str) -> str:
      raw = raw.strip()
      m = re.match(r'^([A-M]\d?)', raw)
      return m.group(1) if m else raw

    def _extract_emotion_code(raw: str) -> str:
      raw = raw.strip()
      m = re.match(r'^(E\d)', raw)
      return m.group(1) if m else raw

    raw_primary = _extract_theme_code(theme_result.get('primary_theme', DEFAULT_THEME))
    raw_surface = _extract_theme_code(theme_result.get('surface_theme', raw_primary))
    ...
    raw_emo = _extract_emotion_code(theme_result.get('dominant_emotion', DEFAULT_EMOTION))
...
# 7. 각 씬(문장)의 이미지 프롬프트 생성
    for i, scene in enumerate(validated_scenes):
        ...
        try:
            ...
            # LLM 호출하여 영문 프롬프트 + pose_type + composition 동시 생성
            prompt_text, pose_type, composition = call_hcx005_image_prompt(sentence_scene_ctx, surface_theme, scene_emotion_code)
