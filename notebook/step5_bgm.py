...
def get_cache_path(poem_dir: Path) -> Path:
  """캐시 경로: step5_bgm.wav"""
  return Path(poem_dir) / 'step5_bgm.wav'
...
def get_total_duration(poem_dir: Path) -> float:
  ...
  """
  schedule_path = Path(poem_dir) / 'step3_sentence_schedule.json'
  ...
  mp3_files = sorted(Path(poem_dir).glob('step2_*_audio.mp3'))
  total = 0.0
...
def run_step5(poem_dir: str, use_cache: bool = True) -> str:
  ...
  입력: {poem_dir}/step1_nlp.json, {poem_dir}/step3_sentence_schedule.json
  출력: {poem_dir}/step5_bgm.wav
  """
...
if __name__ == '__main__':
  ...
  if len(sys.argv) < 2:
    print('사용법: python step5_bgm.py <poem_dir>')
    print('예시: python step5_bgm.py cache/poem_01')
    sys.exit(1)

  poem_dir_arg = sys.argv[1]
  output = run_step5(poem_dir_arg, use_cache=False)
  print(f'BGM 생성 완료: {output}')
