...
7. 행 내 한자 병기 시 (예: 질(質), 중(重)) 한국어만 추출. 해당 줄 자체를 절대 생략하지 말 것.
8. [필수] 이미지에 보이는 모든 텍스트 줄을 위에서 아래로 빠짐없이 추출. 단 한 줄도 생략·건너뜀 금지.
9. 워터마크, 로고, 배경 이미지는 무시. 인쇄된 텍스트만 추출.
10. 굵은 글씨, 밑줄, 색 배경 등 강조 서식은 무시하고 텍스트 내용만 추출.
11. 이미지에 텍스트 구역이 여러 개(상단/하단, 좌/우)로 나뉘어 있으면 위→아래 순서로 모두 연결하여 추출.
"""

OCR_USER_PROMPT = (
  '이미지에 보이는 모든 텍스트를 위 규칙대로 전사하세요.\n'
  '이미지 상단·하단 등 여러 구역에 텍스트가 있으면 전부 위→아래 순서로 포함하세요.\n'
  '이미지에 보이는 텍스트 줄 수와 동일한 줄 수를 출력해야 합니다. '
