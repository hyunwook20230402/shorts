...
2. 한자는 이미지에 있는 그대로 포함하세요. 임의로 괄호 설명을 추가하지 마세요.
3. ①②③ 같은 숫자 원문자, ㉠㉡㉢㉣ 같은 한글 원문자 등 편집자 주석 기호는 모두 제거하세요.
4. (가), (나), (중략) 같은 구조 표지는 그대로 유지하세요.
5. **이미지의 행 구조를 정확히 보존하세요.** 각 줄이 나뉘면 개행(\\n)으로 구분하세요. 절대 한 줄로 이어쓰지 마세요.
6. 작가명·출처·각주가 있으면 본문 뒤에 [출처] 태그와 [각주] 태그로 구분하세요.
   예) [출처] 구암, 『청색곡』
       [각주] * 이사가 고운 원을 파면하고...
7. 마크다운 코드블록(```)으로 감싸지 마세요.
8. 이미지에 2컬럼 이상 레이아웃이 있는 경우:
   - 각 컬럼의 내용을 먼저 파악하세요.
   - 한자/이두 표기로만 구성된 컬럼은 완전히 무시하세요 (위치가 좌/우/중간 어디든 무관).
   - 한국어(현대어 또는 고어) 텍스트가 포함된 컬럼만 추출하세요.
   - 추출 대상 컬럼의 행 순서를 위에서 아래로 정확히 보존하세요.
   - **중요: 두 컬럼을 섞지 마세요. 한국어 컬럼의 행만 위→아래 순서로 순차 추출하세요.**
9. 한 행 안에 한국어 텍스트와 한자/이두 표기가 나란히 병기된 경우:
   - 한국어 부분만 추출하고 한자/이두 표기는 생략하세요.
"""

OCR_USER_PROMPT = '''이미지에 인쇄된 텍스트를 위 규칙에 따라 전사해주세요.

2컬럼 이상 레이아웃인 경우:
- 한자/이두로만 구성된 컬럼은 완전히 무시하세요.
- 한국어 텍스트가 있는 컬럼만 추출하세요.
- 두 컬럼을 섞지 마세요. 한국어 컬럼의 행을 위→아래 순서로만 출력하세요.

예시 (올바른 추출):
입력 이미지: [좌컬럼 한국어] | [우컬럼 한자]
질 좋은 잣이여 | 質(질) 좋은 잣이여
가을에 말러 떨어지지 아니하매, | 秋察尸不冬爾屋隱支障米(...)
→ 출력: "질 좋은 잣이여\n가을에 말러 떨어지지 아니하매,\n"

단일 컬럼이면 행 구조를 그대로 보존하세요.'''
