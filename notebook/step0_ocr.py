...
규칙:
1. 이미지에 인쇄된 모든 텍스트를 추출. 현대어처럼 보여도 이미지 안에 인쇄된 텍스트는 시가 원문으로 간주하여 빠짐없이 추출. AI가 직접 생성한 해설·주석은 출력하지 말 것.
2. ①②③, ㉠㉡㉢ 등 편집자 기호와 (가)(나)(중략) 등 구조 표지 모두 제거.
3. [가장 중요] 행 구조 정확히 보존. 이미지에서 줄바꿈이 있는 곳은 반드시 개행(\\n)으로 구분. 여러 줄을 한 줄로 이어쓰기 절대 금지. 한 연(聯) 안에 여러 행이 있으면 각 행을 별도 줄로 출력.
4. [가장 중요] 동일한 텍스트가 이미지에서 반복 인쇄되어 있으면 반드시 횟수만큼 모두 출력. 중복이라고 판단하여 생략하지 말 것. 고전시가에서는 같은 행을 2회 반복 인쇄하는 것이 정상이다.
5. 출처/각주는 [출처], [각주] 태그로 구분.
6. 코드블록(```) 금지.
7. 2컬럼 레이아웃: 한자/이두 전용 컬럼 무시, 한국어 컬럼만 위→아래 순서로 추출.
8. 행 내 한자 병기 시 (예: 질(質), 중(重)) 한국어만 추출. 해당 줄 자체를 절대 생략하지 말 것.
9. [필수] 이미지에 보이는 모든 텍스트 줄을 위에서 아래로 빠짐없이 추출. 단 한 줄도 생략·건너뜀 금지.
10. 워터마크, 로고, 배경 이미지는 무시. 인쇄된 텍스트만 추출.
11. 굵은 글씨, 밑줄, 색 배경 등 강조 서식은 무시하고 텍스트 내용만 추출.
    [중요] 행 첫머리에 굵은 글씨(볼드) 단어가 있어도 이어지는 일반체 텍스트와 같은 줄로 처리할 것.
    볼드 단어가 있다는 이유만으로 별도 행으로 분리하는 것은 절대 금지.
    판단 기준: 시각적으로 같은 수평 라인에 있으면 같은 행.
12. 이미지에 텍스트 구역이 여러 개(상단/하단, 좌/우)로 나뉘어 있으면 위→아래 순서로 모두 연결하여 추출.

예시 — 이미지에 아래처럼 인쇄되어 있을 때:
...
잘못된 출력 (중복 행 생략 — 절대 금지):
  청산리 벽계수야 수이 감을 자랑 마라
  일도 창해하면 다시 오기 어려워라
"""

OCR_USER_PROMPT = (
  '이미지에 보이는 모든 텍스트를 위 규칙대로 전사하세요.\n'
  '이미지 상단·하단 등 여러 구역에 텍스트가 있으면 전부 위→아래 순서로 포함하세요.\n'
  '먼저 이미지에서 텍스트 행이 몇 줄인지 위에서 아래로 세고, 그 줄 수와 동일한 줄 수를 출력하세요. '
  '현대어처럼 보여도 이미지 안에 인쇄된 텍스트는 모두 추출하세요. '
  '줄을 건너뛰거나 합치지 마세요.\n'
  '[중요] 이미지에서 동일한 텍스트 행이 2줄로 인쇄되어 있으면 출력도 반드시 2줄로 하세요. '
  '이미지에서 시각적으로 별도 줄로 보이는 행은 내용이 같더라도 모두 별도 줄로 출력하세요.'
)
...
if __name__ == '__main__':
  ...
  try:
    from ocr_postcheck import detect_suspicious_lines, format_warnings
    ...
    # 의심 행 자동 감지 경고
    issues = detect_suspicious_lines(result_text)
    if issues:
      cache_path = str(Path(poem_directory) / 'step0' / 'ocr.txt')
      print(format_warnings(issues, cache_path))
      print()
    else:
      print('✅ 의심 행 없음 — Step 1 진행 가능\n')
